...
def findplate(plate):
    try:
        ...
        graphic.setMessage("Vehicle not registered!")
...
def attendancequery(vehicle_id, staff_id):
        ...
        if not record:
            clockedin(vehicle_id, staff_id)
        else:
            print("You already clocked in today")
            graphic.setMessage("You have already clocked in today!")
            gatecontrol()
...
def clockedin(vehicle_id, staff_id):
    ...
    try: 
        ...
        graphic.setMessage("Could not clock in!")
...
#Define the window to display the data
class Gui(threading.Thread):
    ...
    def run(self):
        ...
        #time in label
        self.timeLabel1 = tk.Label(self.root, text='Time in:', bg='white')
        self.timeLabel1.config(font=('helvetica', 30))
        self.canvas1.create_window(105, 500, window=self.timeLabel1)
        #time label
        self.timeLabel2 = tk.Label(self.root, text='', bg='white')
        self.timeLabel2.config(font=('helvetica', 30))
        self.canvas1.create_window(180, 477, window=self.timeLabel2, anchor='nw')
        #location label
